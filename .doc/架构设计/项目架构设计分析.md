...
> 分析日期：2026-07-30 | 仓库：https://gitee.com/volara/easyaiot | 当前分支：main (V9.17.0)
...
| 模块 | 语言/框架 | 职责 | 服务数 |
|------|-----------|------|--------|
| **DEVICE** | Java 21 + Spring Boot 3.5 + Spring Cloud | 设备管理、系统管理、消息推送、数据集、文件存储 | 8+ 微服务 |
...
- **工作流**：Flowable 6.8.0
- **定时任务**：XXL-Job 2.3.1
- **API 文档**：springdoc-openapi 2.8.x + Knife4j 4.5（Jakarta / OpenAPI 3）
...
| **AI/VIDEO 全量镜像构建** | 🟡 中 | 官方 Dockerfile apt/pip 依赖外网；受限网络下可用 localize 镜像 |
| **中间件与业务镜像版本漂移** | 🟡 中 | 需避免混用 Boot2 备份镜像与 Boot3 业务包 |
| **依赖版本管理** | 🟡 中 | 部分依赖版本较旧（如 FastJSON 1.x） |

### 7.2 改进建议

1. **测试体系建设**：补充单元测试、集成测试，建立 CI/CD 流水线
2. **框架跟进**：DEVICE 已升级至 Spring Boot 3.5.16 + JDK 21；持续跟进 Boot/Cloud 小版本与安全补丁
