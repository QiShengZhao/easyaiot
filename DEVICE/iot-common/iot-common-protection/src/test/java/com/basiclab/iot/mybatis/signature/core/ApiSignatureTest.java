package com.basiclab.iot.mybatis.signature.core;
...
public class ApiSignatureTest {
    ...
    @Mock
    private ApiSignatureRedisDAO signatureRedisMapper;
    ...
    public void testSignatureGet() throws IOException {
        ...
        // mock 方法
        when(signatureRedisMapper.getAppSecret(eq(appId))).thenReturn(appSecret);
        ...
        assertTrue(result);
        // 断言调用
        verify(signatureRedisMapper).setNonce(eq(nonce), eq(120), eq(TimeUnit.SECONDS));
    }
